...
import { Franchise, FranchiseList, Role, Store, User, UserList, SimpleUser } from '../service/pizzaService';
...
export default function AdminDashboard(props: Props) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      setUserList(await pizzaService.getUsersList(userPage, 10, '*'));
    })();
  ...
  async function deleteUser(user: SimpleUser) {
    setUserList(await pizzaService.deleteUser(user.id));
  }

  async function filterUsers() {
    setUserList(await pizzaService.getUsersList(userPage, 10, `*${filterUserRef.current?.value}*`));
  }
                ...
                <div className="p-1.5 min-w-full inline-block align-middle">
                  <div className="overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="uppercase text-neutral-100 bg-slate-400 border-b-2 border-gray-500">
                      <tr>
                        <th scope="col" className="px-6 py-3 text-center text-xs font-medium">Name</th>
                        <th scope="col" className="px-6 py-3 text-center text-xs font-medium">Email</th>
                        <th scope="col" className="px-6 py-3 text-center text-xs font-medium">Role</th>
                        <th scope="col" className="pl-6 pr-9 py-3 text-end text-xs font-medium">Action</th>
                      </tr>
                      </thead>
                      {userList.users.map((user, findex) => {
                        return (
                            ...
                            <tr className="border-neutral-500 border-t-2">
                              ...
                              <td className="text-start px-2 whitespace-nowrap text-sm font-normal text-gray-800">{user.roles}</td>

                              <td className="pl-6 pr-2 py-1 whitespace-nowrap text-end text-sm font-medium">
                                <button type="button"
                                        className="w-24 px-2 py-1 inline-flex items-center justify-center text-sm font-semibold rounded-lg border border-1 border-orange-400 text-orange-400  hover:border-orange-800 hover:text-orange-800"
                      ...
                      <tr>
                        <td colSpan={2} className="px-1 py-1">
                          <input type="text" ref={filterUserRef} name="filterUser" placeholder="Filter users"
                        ...
                        <td className="text-end text-sm font-medium">
                          <button
                              ...
                              onClick={() => setUserPage(userPage - 1)} disabled={userPage <= 0}>
                            «
                          </button>
                          <button
                              className="w-12 p-1 text-sm font-semibold rounded-lg border border-transparent bg-white text-grey border-grey m-1 hover:bg-orange-200 disabled:bg-neutral-300"
                              onClick={() => setUserPage(userPage + 1)} disabled={!userList.more}>
                            »
                          </button>
                        </td>
                      </tr>
